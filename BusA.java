...
public class BusA extends Bus {
    ...
    public BusA(String plate, int numberOfRows, int rowOfDoor) {
        super(plate, numberOfRows, 3, rowOfDoor, 1);
        this.busType = "Type A";
    }
    ...
    private boolean canSitAt(int row, int column, Passenger pas) {
        if (seats[row][column].hasTicket()) {
            return false;
        ...
        if (column < SEATS_ON_LEFT_SIDE) { //if the seat is on the left side of the row
            ...
            if (column > 0 && column < SEATS_ON_LEFT_SIDE - 1) {
                return Seat.hasSameGender(pas, seats[row][column + 1])
                        && Seat.hasSameGender(pas, seats[row][column - 1]);
